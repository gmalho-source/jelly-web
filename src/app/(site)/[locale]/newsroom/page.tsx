    ...
    <section className="surface-paper mx-auto max-w-[1200px] px-5 py-16 sm:px-8 lg:py-24">
      ...
      <div className="mt-14 border-t border-line">
        {items.map((item) => (
          <article
            key={item.slug}
            className="grid grid-cols-[78px_minmax(0,1fr)] items-baseline gap-4 border-b border-line py-5 sm:grid-cols-[110px_minmax(0,1fr)_110px] sm:gap-6"
          >
            <time dateTime={item.date} className="text-sm font-semibold tabular-nums text-red">
              {formatter.format(new Date(item.date))}
              <span className="block text-xs font-normal text-fg-soft">{year.format(new Date(item.date))}</span>
            </time>
            <div>
              <h2 className="editorial text-xl lg:text-2xl">{item.title[locale]}</h2>
              {item.summary ? <p className="mt-2 max-w-[62ch] text-sm text-fg-soft">{item.summary[locale]}</p> : null}
              {item.outlet ? <p className="mt-2 text-sm text-fg-soft">{item.outlet}</p> : null}
            </div>
      ...
      <div className="mt-12 flex flex-wrap items-end justify-between gap-6 border-t border-line pt-8">
        <div>
          ...
          <p className="mt-3 max-w-[46ch] text-md text-fg-soft">{t("pressBody")}</p>
        </div>
