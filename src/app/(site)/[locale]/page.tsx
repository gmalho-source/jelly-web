    ...
    <>
      ...
      <header className="surface-ink relative flex min-h-[100svh] flex-col justify-between pt-24">
        ...
        <div className="mx-auto flex w-full max-w-[1600px] flex-wrap items-end justify-between gap-6 border-t border-line px-5 py-6 sm:px-8">
          ...
          <a href="#trabalho" className="flex items-center gap-3 text-xs font-semibold uppercase tracking-[0.1em] text-fg-soft hover:text-fg">
            {t("heroScroll")}
            <span aria-hidden="true" className="grid h-9 w-9 place-items-center rounded-full border border-line">
              ↓
            </span>
          </a>
        </div>
      ...
      <section className="surface-paper">
        <div className="mx-auto flex min-h-[80svh] max-w-[1600px] flex-col justify-center px-5 py-24 sm:px-8">
          ...
          <p className="mt-12 max-w-[26ch] font-display text-[clamp(36px,7vw,116px)] leading-[0.92] tracking-[-0.03em]">
            {t("positionStatement")} <span className="text-red">{t("positionEmphasis")}</span>
          </p>
          <div className="mt-16 flex flex-wrap items-center justify-between gap-6 border-t border-line pt-6">
            <p className="text-xs font-semibold uppercase tracking-[0.1em] text-fg-soft">{t("positionFoot")}</p>
            <p className="text-xs font-semibold uppercase tracking-[0.1em]">{t("positionChain")}</p>
          </div>
        </div>
      </section>
        ...
        <div className="mx-auto max-w-[1600px] px-5 sm:px-8">
          ...
          <div className="mt-8 flex flex-wrap items-end justify-between gap-6">
            <h2 className="font-display text-[clamp(32px,5.4vw,80px)] leading-[0.92] tracking-[-0.03em]">
              ...
              <br />
              <span className="type-outline">{t("workTitleOutline")}</span> {t("workTitleB")}
            </h2>
            ...
            >
              ...
              <span className="relative mx-auto flex max-w-[1600px] flex-col gap-3 px-5 py-9 sm:px-8 lg:flex-row lg:items-center lg:justify-between lg:gap-12 lg:py-12">
                <span className="flex items-baseline gap-5">
                  <span className="text-xs tabular-nums text-fg-soft">0{index + 1}</span>
                  <span className="font-display text-[clamp(28px,4.4vw,60px)] leading-none tracking-[-0.025em]">
                    {service.name[locale]}
                  </span>
                </span>
                <span className="subtitle max-w-[52ch] text-fg-soft">{service.claim[locale]}</span>
              </span>
            </Link>
          ))}
        </div>
      ...
      <section className="surface-red relative overflow-hidden">
        ...
        <div className="relative mx-auto max-w-[1600px] px-5 py-24 sm:px-8 lg:py-32">
          ...
          <dl className="mt-16 grid grid-cols-2 gap-8 border-t border-line pt-8 lg:grid-cols-4">
            {stats.map((stat) => (
              <div key={stat.label}>
                <dt className="font-display text-[clamp(38px,5vw,72px)] leading-none tabular-nums">{stat.value}</dt>
                <dd className="mt-2 text-xs font-semibold uppercase tracking-[0.08em] text-fg-soft">{stat.label}</dd>
              </div>
            ))}
          </dl>
        </div>
      ...
      <section className="surface-paper">
        <div className="mx-auto max-w-[1600px] px-5 py-24 sm:px-8 lg:py-32">
          <div className="flex flex-wrap items-end justify-between gap-6">
            ...
            <Link href="/blog" className="shrink-0 border-b border-fg pb-1 text-sm font-semibold hover:text-red">
      ...
      <section className="surface-ink">
        <div className="mx-auto max-w-[1600px] px-5 py-24 sm:px-8 lg:py-32">
          ...
          <Link href="/contactos" className="mt-8 inline-block rounded-full bg-red px-6 py-3.5 font-semibold text-white transition-colors duration-200 hover:bg-red-deep">
            {t("contactCta")}
          </Link>
        </div>
      </section>
