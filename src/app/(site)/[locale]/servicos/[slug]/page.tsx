...
export default async function ServicePage({ params }: { params: Promise<Params> }) {
  ...
  return (
    <>
      ...
      {service.promise ? (
        <section className={`py-14 lg:py-20 ${accent ? "bg-lavender text-ink" : "bg-ink text-paper"}`}>
          <div className="mx-auto max-w-[1200px] px-5 sm:px-8">
            <span className={`eyebrow ${accent ? "text-red-deep" : "text-chartreuse"}`}>{t("promise")}</span>
            <p className={`mt-4 max-w-[34ch] font-display text-chapter ${accent ? "text-ink" : "text-paper"}`}>
          ...
          <div>
            ...
            <ul className="mt-5 flex flex-col gap-3">
              {service.includes?.map((item) => (
                <li key={item.pt} className="flex gap-3 text-md text-slate">
          ...
          <div>
            ...
            <ol className="mt-5 border-t border-ink">
              {service.phases?.map((phase, index) => (
                <li key={phase.name.pt} className="grid grid-cols-[36px_minmax(0,1fr)] gap-4 border-b border-paper-2 py-5">
                  <span className="font-sans text-xs font-semibold tabular-nums text-red">
                    {String(index + 1).padStart(2, "0")}
                  </span>
                  <div>
                    <h3 className="text-xl">{phase.name[locale]}</h3>
                    <p className="mt-2 max-w-[56ch] text-sm text-slate">{phase.body[locale]}</p>
                  </div>
                </li>
              ))}
            </ol>
          ...
          <div className="mt-5 border-t border-ink">
            {cases.map((project) => (
              ...
              >
                ...
                <span className="hidden text-sm text-mute sm:block">{project.title[locale]}</span>
        ...
        <div className="mt-5 grid gap-4 sm:grid-cols-3">
          {others.map((item) => (
            ...
            >
              ...
              <p className="text-sm text-slate">{item.claim[locale]}</p>
