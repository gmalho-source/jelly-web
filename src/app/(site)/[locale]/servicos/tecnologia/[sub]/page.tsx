...
export async function generateMetadata({ params }: { params: Promise<{ locale: Locale; sub: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: { type: "website", title: servico.nome[locale], description: servico.descricao[locale], images: [{ url: `${SITE_URL}${servico.topo?.poster.src ?? tecnologia.topo.poster.src}` }] },
  };
}
